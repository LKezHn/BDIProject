...
from .LoadDrawWindow import LoadDrawWindow
from .SaveDrawWindow import SaveDrawWindow
...
class DrawingWindow(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        def loadFile():
			...
			# # Comprobamos que halla selecionado un archivo
            # if(filename):
                #Limpiamos el lienzo
            newWindow()

            # Reinicializamos los comandos usados para una nueva imagen. 
            self.graphicsCommands = PyList()
            # Esta funcion creara la lista de comandos a leer.
            ...
            # Actualiza la ventana despues de divbujar la imagen.
            screen.update()
		...
		#	Este metodo se encarga de dibujar un circulo.
        ...
        if(self.user['isAdmin']):
            penEntry = tkinter.Entry(sideBar,textvariable=penColor, state='readonly')
        else:
            penEntry = tkinter.Entry(sideBar,textvariable=penColor, state='disabled')
        penEntry.pack()
        penColor.set(am.getPenColor())  
        
        def getPenColor():
            color = tkinter.colorchooser.askcolor()
            if color != None:
                penColor.set(str(color)[-9:-2])
                penColor.trace("w",am.changeColors(penColor.get(), fillColor.get()))
        
        if(self.user['isAdmin']):
            penColorButton = tkinter.Button(sideBar, text = "Pick Pen Color", command=getPenColor)
        else:
            penColorButton = tkinter.Button(sideBar, text = "Pick Pen Color", command=getPenColor, state='disabled')
        ...
        if(self.user['isAdmin']):
            fillEntry = tkinter.Entry(sideBar,textvariable=fillColor, state='readonly')
        else:
            fillEntry = tkinter.Entry(sideBar,textvariable=fillColor, state='disabled')
        fillEntry.pack()
        fillColor.set(am.getFillColor())     
        
        def getFillColor():
            color = tkinter.colorchooser.askcolor()
            if color != None:    
                fillColor.set(str(color)[-9:-2])
                fillColor.trace("w",am.changeColors(penColor.get(), fillColor.get()))       
   
        if(self.user['isAdmin']):
            fillColorButton = tkinter.Button(sideBar, text = "Pick Fill Color", command=getFillColor)
        else:
            fillColorButton = tkinter.Button(sideBar, text = "Pick Fill Color", command=getFillColor, state='disabled')
